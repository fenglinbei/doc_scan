...
```json
{
  ...
  "artifacts": {
    ...
    "binary_niblack": "/api/results/{job_id}/binary_niblack.png",
    "binary_wolf": "/api/results/{job_id}/binary_wolf.png",
    "binary_nick": "/api/results/{job_id}/binary_nick.png",
    "binary_bradley": "/api/results/{job_id}/binary_bradley.png",
    "binary_wolf_fused": "/api/results/{job_id}/binary_wolf_fused.png",
...
- 中间结果网格：角点检测、透视矫正、低对比增强、可读二值化、固定阈值、Otsu、Sauvola、Niblack、Wolf、NICK、Bradley、Ours (`binary_wolf_fuse`)、最终结果。
...
当前已实现 DIBCO2019 Track A、DIBCO2019 Track B 和 H-DIBCO2018 的客观二值化评测脚本：

```bash
python3 experiments/evaluate_dibco.py --out runtime/experiments/dibco_eval_v1
```

该脚本跳过角点定位和透视矫正，直接复用后端 `enhance_and_binarize` 评估 `binary_fixed`、`binary_otsu`、`binary_sauvola` 和 `binary_readable`。输出包含每图 artifact、contact sheet、逐图指标和聚合指标。详细说明见 `docs/dibco_hdibco_evaluation.md`。
...
- DIBCO/H-DIBCO 二值化客观指标：Precision、Recall、F1、pseudo-F1、IoU、PSNR、NRM、DRD、前景比例误差。
