...
1. 对矫正图转灰度。
2. 使用大核闭运算估计背景光照场，默认核大小 `45x45`。
3. 用 `gray / background * 255` 做除法归一化，降低阴影影响。
4. 对归一化图像执行：
   - 顶帽变换，增强局部亮细节。
   - 底帽变换，提取暗文字和阴影结构。
5. 使用 `illumination_corrected - blackhat + tophat` 得到形态学增强图。
...
- Sauvola：局部自适应阈值，默认窗口 `35`、`k=0.2`，作为默认最终结果。
...
- 中间结果网格：角点检测、透视矫正、形态学增强、固定阈值、Otsu、Sauvola、最终结果。
...
- 结果图先加载小尺寸预览或关键结果：`corner_detection`、`rectified`、`morphology_enhanced`、`final` 优先，其余 artifact 延迟加载。
